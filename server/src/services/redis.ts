...
//
// ── Key Namespaces ────────────────────────────────────────────────────────────
//   lobby:{code}:meta            → JSON string of LobbyMeta
...
export const keys = {
  // Lobby metadata (ownerId, maxPlayers, status, etc.)
  lobbyMeta: (code: string) => `lobby:${code}:meta`,
...
export async function getLobbyMeta(code: string): Promise<LobbyMeta | null> {
  const raw = await redis.get(keys.lobbyMeta(code));
  if (!raw) return null;
  return JSON.parse(raw) as LobbyMeta;
}

export async function setLobbyMeta(code: string, meta: LobbyMeta): Promise<void> {
  await redis.set(keys.lobbyMeta(code), JSON.stringify(meta), "EX", LOBBY_TTL);
}
...
// Delete all lobby keys (cleanup when a lobby closes)
export async function deleteLobby(code: string): Promise<void> {
  await Promise.all([
    redis.del(keys.lobbyMeta(code)),
    redis.del(keys.lobbyPlayers(code)),
  ]);
}
