...
router.post("/login", authLimiter, async (req, res) => {
  ...
  try {
    ...
    // IMPORTANT: Return the same error message whether the email or password is wrong.
    // Distinct messages ("email not found" vs "wrong password") let attackers
    // enumerate valid email addresses. Never reveal which one was wrong.
    if (!user) {
